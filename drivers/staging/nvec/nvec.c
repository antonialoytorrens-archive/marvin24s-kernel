...
static void nvec_writel(struct nvec_chip *nvec, u32 val, unsigned long reg)
{
...
static int __devinit tegra_nvec_probe(struct platform_device *pdev)
{
	...
	if (nvec->nvec_devices) {
		ret = mfd_add_devices(nvec->dev, -1, nvec->nvec_devices,
				nvec->nr_nvec_devs, nvec->base, 0);
		if (ret)
			dev_err(nvec->dev, "error adding subdevices\n");
		else
			dev_info(nvec->dev, "added %d NVEC subdevices\n",
				 nvec->nr_nvec_devs);
	}
