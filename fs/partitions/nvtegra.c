...
int nvtegra_partition(struct parsed_partitions *state)
{
	...
	printk(KERN_WARNING "nv-part: nvtegra_partition()\n");
	printk(KERN_WARNING "nv-part: bdev_hardsect_size() = %d\n", bdev_logical_block_size(state->bdev));
        */

	pt = kzalloc(2048, GFP_KERNEL);
	if (!pt) {
	    printk(KERN_WARNING "nv-part: error kzalloc\n");
	    return -1;
	}

	sector = 1536*4-0x1000;
	if (read_dev_bytes(state->bdev, sector, (char*)pt, 2048) != 2048) {
		printk(KERN_WARNING "nv-part: error read_dev_bytes\n");
		goto out;
	}
	...
	while ((p->id < 128) && (count<=24)) {
		...
		printk(KERN_WARNING "nv-tegra: nvtegrapart: %d [%s] %d %d %d\n", id, name, type, start, size);

		start = start*4 - 0x1000;
		size = size*4;

		printk(KERN_WARNING "nv-tegra: --> put_partition(%d,%d,%d)\n", count, start, size);
